...
function loadBot(options, prechatFormDetails, prechatInfo){
    ...
    var minMaxObserver = new MutationObserver(function (mutations, me) {
        mutations.forEach(function(mutation) {
            ...
            if (mutation.target.className.indexOf('sidebarMaximized') != -1) {
                ...
                //var height = 100;
                //var height = 100;
                console.log('minimized height: ' + height)
                var width = embedded_svc.settings.widgetWidth;
                window.parent.postMessage({
                    frameHeight: height,
                    frameWidth: width,
                    parentMarginRight: 70
    ...
    function helpButtonLoaded() {
        ...
        if (!helpButton) {
            window.setTimeout(helpButtonLoaded, 2000);
            return;
        }
    ...
    var initESW = function(gslbBaseURL, buttonId, deploymentId) {
        ...
        embedded_svc.settings.avatarImgURL = options.contentURL + eswOptions.resourceBase + '/img/cta-icon.png';
        embedded_svc.settings.chatbotAvatarImgURL = options.contentURL + eswOptions.resourceBase + '/img/cta-icon.png';
